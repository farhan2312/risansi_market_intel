...
import { ZONE_BG, ZONE_COLOR, type RepData } from './RepRow';
...
export function RouteRow({ route, reps }: { route: RouteData; reps: RepData[] }) {
  const [editing, setEditing]         = useState(false);
  const [selectedRep, setSelectedRep] = useState(route.rep_id ? String(route.rep_id) : '');
  ...
  return (
    <tr style={{ borderBottom: '1px solid var(--line)' }}>
      ...
      <td style={TD}>
        <span style={{ padding: '2px 7px', borderRadius: 10, fontSize: 11, fontWeight: 500, background: ZONE_BG[route.zone] ?? 'var(--bg-sunk)', color: ZONE_COLOR[route.zone] ?? 'var(--fg-3)' }}>
          {route.zone}
        </span>
      </td>
      <td style={TD}>
